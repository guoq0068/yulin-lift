...
const { byteSize, intSize, floatSize, vector3Size, quaternionSize, adjustAngleSize } = require('./typeSize');
 ...
 * @param {*} socket 
 */
function sendAdjustAngle(socket) {
    const buffer = Buffer.allocUnsafe(byteSize + intSize + adjustAngleSize);
    const byteWriter = new ByteWriter(buffer);
    byteWriter.writeByte(MessageType.AdjustAimAngle);
    byteWriter.writeInt(socket.clientID);
    const angle = new AdjustAngle(10, 5, 1);
    byteWriter.writeAdjustAngle(angle);
    console.log(buffer);
    socket.write(buffer);
}

function sendString(socket) {
    console.log("send String is called");
    const string = "this is 我们 test";
    const stringBuf = Buffer.from(string);
    const buffer = Buffer.allocUnsafe(byteSize + intSize * 2 + stringBuf.length);
    const byteWriter = new ByteWriter(buffer);
    byteWriter.writeByte(MessageType.MessageString);
    byteWriter.writeInt(socket.clientID);
    byteWriter.writeInt(stringBuf.length);
    stringBuf.copy(buffer, byteSize + intSize * 2);
